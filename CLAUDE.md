...
- **Flight plan** (departure/destination airport) comes from `FlightPlanReceived`/`RequestFlightPlan()` on `IBroker`. Needs empirical verification that this still works now that vPilot removed its own in-client filing UI in favor of the VATSIM website — if it's dead, fall back to fetching directly from the SimBrief API.
